...
var ASM_CONSTS = [function() { above (); },
 function($0) { { submit (Pointer_stringify ($0)); } }];
...
STATICTOP = STATIC_BASE + 205558800;
...
Module.asmLibraryArg = { "abort": abort, "assert": assert, "enlargeMemory": enlargeMemory, "getTotalMemory": getTotalMemory, "abortOnCannotGrowMemory": abortOnCannotGrowMemory, "invoke_ii": invoke_ii, "invoke_iiii": invoke_iiii, "_emscripten_asm_const_ii": _emscripten_asm_const_ii, "___lock": ___lock, "___syscall6": ___syscall6, "___setErrNo": ___setErrNo, "_abort": _abort, "___syscall140": ___syscall140, "_emscripten_memcpy_big": _emscripten_memcpy_big, "___syscall54": ___syscall54, "___unlock": ___unlock, "_emscripten_asm_const_v": _emscripten_asm_const_v, "_exit": _exit, "__exit": __exit, "___syscall146": ___syscall146, "DYNAMICTOP_PTR": DYNAMICTOP_PTR, "tempDoublePtr": tempDoublePtr, "ABORT": ABORT, "STACKTOP": STACKTOP, "STACK_MAX": STACK_MAX };
...
var _mine = Module["_mine"] = function() { return Module["asm"]["_mine"].apply(null, arguments) };
...
function speed() {
  var cur = new Date();
  var tot = stat_above + stat_sent;
  var tim = (cur - start_time) / 1000;

  if (start_time && tim > 0) {
    minespeed = Number(tot / tim).toFixed(2);
  }
  if (job_time)
    job_time = (cur - job_time) / 1000 | 0;
}
...
onmessage = function(e) {
  if (pid === -1) {
    pid = e.data.id;
  } else if (e.data === "get_status") {
    postMessage({ id: pid, found: stat_above, speed: minespeed, submitted: stat_sent});
  }
}
